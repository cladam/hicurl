...
pub fun parse_items(args: list<string>) {
...
pub fun parse_rest(req: RequestSpec, first_arg: string, rest: list<string>) {
...
pub fun parse_operator(req: RequestSpec, item: string) =>
  match index_of(item, "==") {
    Some(idx) => {
      let name = item[0:idx]
      let val = item[idx+2:]
    ...
    None => match index_of(item, ":=@") {
      Some(idx) => {
        let name = item[0:idx]
        let file_path = item[idx+3:]
        let val = match read_file(file_path) {
          Ok(c) => c,
          Err(_) => ""
        }
